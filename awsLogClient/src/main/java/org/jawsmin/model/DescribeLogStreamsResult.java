 ...
 */
public class DescribeLogStreamsResult extends JsonMapperModel
		implements
			Serializable,
			Cloneable {
	...
	public JsonMapperModel toModel(String json) {
		...
		logsArrayJson.forEach(jsonObj -> {
			...
			if (jsonObj.asObject().get("logStreamName") != null)
				log.setLogStreamName(
						jsonObj.asObject().get("logStreamName").asString());
			...
			if (jsonObj.asObject().get("firstEventTimestamp") != null)
				log.setFirstEventTimestamp(
						jsonObj.asObject().get("firstEventTimestamp").asLong());

			if (jsonObj.asObject().get("lastEventTimestamp") != null)
				log.setLastEventTimestamp(
						jsonObj.asObject().get("lastEventTimestamp").asLong());

			if (jsonObj.asObject().get("lastIngestionTime") != null)
				log.setLastIngestionTime(
						jsonObj.asObject().get("lastIngestionTime").asLong());

			if (jsonObj.asObject().get("uploadSequenceToken") != null)
				log.setUploadSequenceToken(jsonObj.asObject()
						.get("uploadSequenceToken").asString());
