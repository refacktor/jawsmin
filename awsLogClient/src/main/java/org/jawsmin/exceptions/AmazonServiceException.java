package org.jawsmin.exceptions;
...
public class AmazonServiceException extends RuntimeException {
	...
	/**
	 * Track proxy host if configured, in case error response came from proxy
	 * instead of AWS.
	 */
	...
	public String getMessage() {
		return getErrorMessage() + " (Service: " + getServiceName()
				+ "; Status Code: " + getStatusCode() + "; Error Code: "
				+ getErrorCode() + "; Request ID: " + getRequestId()
				+ "; Proxy: " + getProxyHost() + ")";
	}

	/**
	 * Typically only useful for debugging purpose if for some reason the SDK
	 * cannot parse the HTTP response from a service
	 *
	...
	public void setRawResponseContent(String rawResponseContent) {
		this.rawResponse = rawResponseContent == null
				? null
				: rawResponseContent.getBytes(StandardCharsets.UTF_8);
	}
	...
	/**
	 * Returns proxy host if configured. If using a proxy, then it's possible
	 * that the error response came from the proxy instead of AWS.
	 *
