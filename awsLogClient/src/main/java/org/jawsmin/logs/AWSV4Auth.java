...
public class AWSV4Auth {
	...
	private String prepareCanonicalRequest() {
		 ...
		 * character.
		 */
		canonicalURI = canonicalURI == null || canonicalURI.trim().isEmpty()
				? "/"
				: canonicalURI;
		...
		if (queryParametes != null && !queryParametes.isEmpty()) {
			for (Map.Entry<String, String> entrySet : queryParametes
					.entrySet()) {
				String key = entrySet.getKey();
				String value = entrySet.getValue();
				queryString.append(key).append("=")
						.append(encodeParameter(value)).append("&");
			}

			/*
			 * @co-author https://github.com/dotkebi @git #1 @date 16th March,
			 * 2017
			 */
		...
		strSignedHeader = signedHeaders.substring(0,
				signedHeaders.length() - 1); // Remove last ";"
	...
	private String prepareStringToSign(String canonicalURL) {
		 ...
		 * character.
		 */
		stringToSign += currentDate + "/" + regionName + "/" + serviceName + "/"
				+ aws4Request + "\n";
	...
	private String calculateSignature(String stringToSign) {
		try {
			...
			byte[] signatureKey = getSignatureKey(secretAccessKey, currentDate,
	...
	private String buildAuthorizationString(String strSignature) {
		return HMACAlgorithm + " " + "Credential=" + accessKeyID + "/"
				+ getDate() + "/" + regionName + "/" + serviceName + "/"
				+ aws4Request + "," + "SignedHeaders=" + strSignedHeader + ","
